...
export class NewComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.filteredOptions = this.destinatario.valueChanges.pipe(
      startWith(''),
    ...
    this._userService
      .getUsers()
      .subscribe((e) => (this.options = e.map((user) => user.userName)));
  }
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == true) {
        ...
        this._memorandumService
          .sendMemorandum(this.memorandum, this.destinatario.value)
          .subscribe((result) => {
            console.log(`Enviado!`);
          });
